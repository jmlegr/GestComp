...
from treebeard.ns_tree import NS_NodeQuerySet, NS_Node, NS_NodeManager
...
class MultiNS_NodeQuerySet(models.query.QuerySet):
    """
    ...
    def delete(self, removed_ranges=None):
        ...
        if removed_ranges is not None:
            ...
            # foreign keys...
            super(MultiNS_NodeQuerySet, self).delete()
            cursor = connection.cursor()
...
class MultiNS_NodeManager(models.Manager):
    
    def get_query_set(self):
        "Sets the custom queryset as the default."
        print "multins query"
        return MultiNS_NodeQuerySet(self.model).order_by('tree_id', 'lft')
...
class testManager(models.Manager):
    ...
    def get_query_set(self):
        ...
        if self.tree is None:
            print "test sans tree"
            return NS_NodeQuerySet(self.model).order_by('tree_id', 'lft')
        #try:
        print "test avec tree",self.tree
        return MultiNS_NodeQuerySet(self.model).filter(tree_id1=self.tree).order_by('tree_id1', 'lft1')
        #except:
        #    raise("erreur sur arbre %s" %self.tree)
    ...
    def with_counts(self):
        ...
        print 'pp',cursor
        cursor.execute("""
            SELECT p.id, p.tree_id,p.nom
            FROM competences_competence p
            where id>10 and p.tree_id1= %s
            """ % self.tree) 
        result_list = []
        #print cursor.fetchall()
        for row in cursor.fetchall():
            print('row',row)
            p = self.model(id=row[0], tree_id=row[1])
            p.nom = row[2]
            result_list.append(p)
        return result_list
    
class MultiNS_Node(NS_Node):
    tree_id1=models.PositiveIntegerField()
    lft1=models.PositiveIntegerField()
    rgt1=models.PositiveIntegerField()
    test1=testManager()
    test2=MultiNS_NodeManager()
    objects=NS_NodeManager()
     
    def get_ancestors(self,tree=None):
        if tree is None:
            print "asc sans tree"
            return super(MultiNS_Node,self).get_ancestors()
        else:
            print 'all avec tree',tree
            return self.__class__.objects.filter(
                                                 tree_id1=tree,
                                                 lft1__lt=self.lft1,
                                                 rgt1__gt=self.rgt1)
